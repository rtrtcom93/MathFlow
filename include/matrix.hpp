...
Matrix<T>::Matrix(size_t nrow, size_t ncol) 
    : mat(nrow, Tensor<T>(ncol)) {
        //std::cout << "Initialize with size" << std::endl;
    } 

template<typename T> 
Matrix<T>::Matrix(size_t nrow, size_t ncol, T init_val) 
    : mat(nrow, Tensor<T>(ncol, init_val)) {
...
//Accessor operators
template<typename T>
Tensor<T>& Matrix<T>::operator[](size_t index) {
    return mat[index];
}

template<typename T>
const Tensor<T>& Matrix<T>::operator[](size_t index) const {
    return mat[index];
}
...
std::istream &operator>>(std::istream &is, Matrix<U> &rhs) {
    ...
    while (std::getline(is, line)) {  // 여러 줄 입력 처리
        ...
        std::vector<U> row;  // 각 행에 해당하는 Tensor<T>
        U value;
        while (iss >> value) {
...
template<typename U>
Tensor<U> operator*(const Tensor<U> &lhs, const Matrix<U> &rhs) {
// #ifdef DEBUG 
...
template<typename U>
Matrix<U> matmul(const Tensor<U> &lhs, const Matrix<U> &rhs) {
// #ifdef DEBUG 
    if (lhs.size() != rhs.nrow()) 
        throw std::invalid_argument("Matrix multiplication error: V.size() = " +
                                    std::to_string(lhs.size()) + ", M.nrow() = " +
...
template<typename T>
Tensor<T> Matrix<T>::upp_diag(size_t upper) const{
    size_t ndiag=
        (this->ncol() >= this->nrow()) ? 
        this->nrow() : this->ncol();
    if (upper >= this->ncol())
...
template<typename T>
Tensor<T> Matrix<T>::low_diag(size_t lower) const{
    size_t ndiag=
        (this->ncol() >= this->nrow()) ? 
        this->nrow() : this->ncol();
    if (lower >= this->nrow())
