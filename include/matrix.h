...
public :
    ...
    Matrix<T> operator*(const Matrix<T> &rhs) const;
    template<typename U>
    friend Tensor<U> operator*(const Tensor<U> &lhs, const Matrix<U> &rhs);
    template<typename U>
    friend Matrix<U> matmul(const Matrix<U> &lhs, const Matrix<U> &rhs);
    template<typename U>
    friend Tensor<U> matmul(const Tensor<U> &lhs, const Matrix<U> &rhs);
    ...
    Tensor<T> diag() const;
    Tensor<T> upp_diag(size_t upper) const;
    Tensor<T> low_diag(size_t lower) const;
